...
CREATE TABLE `__new_knowledge_item` (
	...
	CONSTRAINT "knowledge_item_type_status_check" CHECK(
        ("__new_knowledge_item"."type" IN ('file', 'url', 'note') AND "__new_knowledge_item"."status" != 'preparing')
        OR ("__new_knowledge_item"."type" IN ('directory', 'sitemap') AND "__new_knowledge_item"."status" NOT IN ('reading', 'embedding'))
      ),
...
INSERT INTO `__new_knowledge_item`("id", "base_id", "group_id", "type", "data", "status", "error", "created_at", "updated_at")
SELECT
	"id",
	"base_id",
	"group_id",
	...
	END,
	CASE
		WHEN "status" = 'processing' THEN 'Knowledge item was interrupted during lifecycle schema migration.'
		ELSE "error"
	END,
	"created_at",
	"updated_at"
FROM `knowledge_item`;--> statement-breakpoint
